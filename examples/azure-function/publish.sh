...
# own license terms, so the SDK is never checked in; rapplication jobs publish it inside each code app.
set -eu
APP="$1"
SDK="${2:-../../../copilot-harness-sdk}"
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"
rm -rf brainfreeze_studio sdk .python_packages codeapp-host.js
